...
bool HttpRequestMgr::init(){
    ...
    if(curl_global_init(CURL_GLOBAL_ALL)) {
        G_LOG_FC(LOG_ERROR, "Could not init cURL\n");
        return false;
    }
        ...
        switch(message->msg) {
            case CURLMSG_DONE:{
                ...
                if (curl_code!=CURLE_OK){
                    G_LOG_FC(LOG_ERROR, "CURLINFO_PRIVATE failed CURLE:%d %s",curl_code,curl_easy_strerror(curl_code));
                }
                if (http_request)
            ...
            default:
                G_LOG_FC(LOG_ERROR, "CURLMSG default\n");
                break;
        }
    }
}
...
int HttpRequest::setRequestUrl(const std::string& url){
    ...
    if (curl_code!=CURLE_OK){
        G_LOG_FC(LOG_ERROR, "CURLOPT_URL failed CURLE:%d %s",curl_code,curl_easy_strerror(curl_code));
    }
    return curl_code;
}
...
int HttpRequest::setRequestTimeout(long timeout){
    ...
    if (curl_code!=CURLE_OK){
        G_LOG_FC(LOG_ERROR, "CURLOPT_TIMEOUT_MS failed CURLE:%d %s",
...
int HttpRequest::setPostData(const void* data, unsigned long size){
    ...
    if (curl_code!=CURLE_OK){
        G_LOG_FC(LOG_ERROR, "CURLOPT_POST failed CURLE:%d %s",curl_code,curl_easy_strerror(curl_code));
        return curl_code;
    }
    ...
    if (size == 0) {
        ...
        if (curl_code!=CURLE_OK){
            G_LOG_FC(LOG_ERROR, "CURLOPT_POSTFIELDS failed CURLE:%d %s",
                    curl_code,curl_easy_strerror(curl_code));
            return curl_code;
        }
    }
    ...
    if (curl_code!=CURLE_OK){
        G_LOG_FC(LOG_ERROR, "CURLOPT_POSTFIELDSIZE failed CURLE:%d %s",
                curl_code,curl_easy_strerror(curl_code));
    }
